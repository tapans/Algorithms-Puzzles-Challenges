...
class Mosaic(object): 
    ...
    def create_mosaic(self, filename, min_size):
        ...
        picture = Image.open(filename) # apply methods of Image module
                
        def _helper_mosaic(self, image, min_size):
            ''' Help parent function create a photomosaic version of a single
            pict by applying the algorithm on portions of the original pic '''
            ...
            if width >= min_size or height >= min_size:
                ...
                result_a = _helper_mosaic(self, quad_a, min_size) 
                result_b = _helper_mosaic(self, quad_b, min_size) 
                result_c = _helper_mosaic(self, quad_c, min_size)
                result_d = _helper_mosaic(self, quad_d, min_size) 
                ...
                #paste the resulting parts on positions starting from the
                #upper left coordinate specified for each quadrant.
                          
            else: ##base case for recursion
                # find a closest match from database
                # resize the closest match to fit the size of the pic component
                # paste the closest match onto the picture component
            
                d = {} 
                img_avg_color = _avg_color(image) ## faster average function?
        ...
        self.picture = _helper_mosaic(self, picture, min_size)        
        #the modified picture is now the original picture.
...
if __name__ == '__main__':
    x = time.time()
    m = Mosaic('images')
    m.create_mosaic('original.jpg', 20)
    m.save_as('mosaic_raw')
    print time.time() - x